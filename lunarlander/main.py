...
def training(name="intial_model", render=False):
    ...
    PATH = f"models/{name}"
    dqn = DQN("LunarLander-v2", REPLAY_BUFFER_SIZE, LEARNING_RATE, DISCOUNT_FACTOR, ARCHITECTURE)
    train_reward = dqn.train(epsilon_decay=EPSILON_DECAY, num_episodes=NUM_EPISODES, batch_size=BATCH_SIZE, target_update_freq=TARGET_UPDATE_FREQ, output=100, saving=PATH, render=render)
...
BATCH_SIZES = [32, 64]
TARGET_UPDATE_FREQS = [1, 4, 8, 16, 32]

#training and testing
# training(name="PLACEHOLDER_NAME", render=True)
